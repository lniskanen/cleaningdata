...
## mergedDataSet table variables

Variables (4:564) as in /UCI HAR Dataset/activity_labels.txt
...
### Activity and ActivityName variables
...
* 1..30 - number of the test subject (person)




## meandSTdsTable  variables
...
* Subject
* Activity
* 4...564 variable means for given activity and subject

### tidyTB observerations
* 180 observations for earch value of ActivityName and Subject

# Transformations 

1. X_test.txt and X_train.txt merged
2. features.txt column names extracted and placed into merged table called mergedDataSet
3. meansStdsTable created by selecting only columns with "mean" or "std" found in column names
4. subject_test.txt,subject_train.txt, y_test.txt, y_train.txt data merged into the mergedDataSet
5. Subject and Activity variable names added into the mergedDataSet added columns
6. ActivityName list variable with human readable lables created into the mergedDataSet 
7. mergeDataSet column means are calculated into tidyTB table for each given Activity and Subject